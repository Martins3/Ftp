...
TcpServer::TcpServer(IOEventLoop *loop, SocketAddr &addr)
    : eventLoop(loop), tcpAddr(addr), tcpAccept(new TcpAccept(loop, addr)),
      isStart(false), threadPool(new IOEventLoopThreadPool(loop)) {
  tcpAccept->setNewConnectCallback(std::bind(&TcpServer::newConnected, this,
                                             std::placeholders::_1,
                                             std::placeholders::_2));
}
...
void TcpServer::newConnected(int sockfd, const SocketAddr &addr) {
  base::Log::OUT(base::Log::Info)
      << "new tcp connect addr:" << addr.toString()
      << "<cnt>:" << std::to_string(getConnectCount());

  IOEventLoop *loop = threadPool->getOneLoopFromPool();
  std::shared_ptr<TcpConnect> tcpConnect(
      new TcpConnect(loop, addr.getAddr(), sockfd));
  ...
  tcpConnect->setCloseCallback(
      std::bind(&TcpServer::connectCloseEvent, this, std::placeholders::_1));
...
void TcpServer::removeConnect(string name) { connectPool.erase(name); }

void TcpServer::connectCloseEvent(std::shared_ptr<TcpConnect> connect) {
  // 1. 是不是将 SSL 的释放 放在此处更好 ？
  connectCloseCallback(connect);
  removeConnect(connect->getName());
}

long TcpServer::getConnectCount() { return connectPool.size(); }
...
void TcpServer::write(std::shared_ptr<TcpConnect> connect, void *data,
                      uint32_t length) {
