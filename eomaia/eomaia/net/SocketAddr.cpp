...
SocketAddr::SocketAddr(const char * hostname, uint16_t port, bool test) : valid(false) {
  struct sockaddr_in addrIn;
  if (!SocketOperation::toAddrIpv4T(hostname, port, addrIn)) {
    return;
  }
  new (this) SocketAddr(addrIn);
}
...
bool SocketAddr::isValid() { return valid; }
