...
int SocketOperation::createTestingSocket() {
...
// 为了 SSL ，这里的函数只能被 Socket 调用
ssize_t SocketOperation::read(int sockfd, void *buf, size_t count) {
  return ::read(sockfd, buf, count);
}

ssize_t SocketOperation::readv(int sockfd, const struct iovec *iov,
                               int iovcnt) {
...
void SocketOperation::getAddrAnyIpv4(struct sockaddr_in &addrIn,
                                     uint16_t port) {
...
bool SocketOperation::toAddrIpv4(const string &addrIp,
...
bool SocketOperation::toAddrIpv4T(const char *hostname, int port, struct sockaddr_in &addr) {
  int sd;
  struct hostent *host;
...
bool SocketOperation::toAddrIpv4(const string &addr, uint16_t port,
                                 struct sockaddr_in &addrIn) {
  vector<string> ip;
  size_t first = 0;
  while (true) {
