...
const uint32_t Buffer::InitSize = 165536;
...
uint32_t Buffer::writableBytes() { return buffer.size() - writeIndex; }

uint32_t Buffer::readableBytes() const { return writeIndex - readIndex; }

int Buffer::readFromIO(Socket * socket, int &errorNo){
  uint32_t writable = writableBytes();
  auto base = &(*buffer.begin()) + writeIndex;

  int n = socket->read(base, writable);
  if (n < 0) {
    errorNo = errno;
  } else if ((uint32_t)n <= (writable - 10)) {
    writeIndex += n;
  }else {
    base::Log::OUT(base::Log::Error) << "Maybe setup a bigger buffer !\n";
  }
  return n;
}
