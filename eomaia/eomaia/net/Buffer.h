...
public:
  ...
  int readFromIO(Socket * socket, int &errorNo);

  uint32_t writableBytes();
  uint32_t readableBytes() const;
  bool isReadble() { return readableBytes() > 0 ? true : false; }

  bool isEmpty() { return !isReadble(); }
  ...
  const uint8_t *readIndexPtr() const { return bufferPtr() + readIndex; }
  ...
  Message readAllAsMessage() {
    auto len = readableBytes();
    char * data = (char *)malloc(sizeof(char) * len);
    memcpy(data, readIndexPtr(), len);
  ...
  const char *findCRLF() const {
    ...
    const char *crlf =
        std::search((char *)readIndexPtr(), (char *)bufferPtr() + writeIndex,
                    kCRLF, kCRLF + 2);
