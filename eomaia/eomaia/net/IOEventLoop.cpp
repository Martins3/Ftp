...
class IgnoreSigPipe {
public:
  static IgnoreSigPipe *getInstance() { return singnal; }
...
IOEventLoop::IOEventLoop()
    : eventCtrl(new IOEventCtrl(this)), thisTheadId(Thread::getNowThreadId()),
      timerQueue(new TimerQueue(this)) {}

IOEventLoop::~IOEventLoop() { delete eventCtrl; }
...
void IOEventLoop::run() {
  ...
  if (!inThisThread()) {
    ...
    base::Log::OUT(base::Log::Warn)
        << "Loop create and run in different thread\n";
  }
...
void IOEventLoop::removeEvent(int fd) { eventCtrl->deleteEvent(fd); }

void IOEventLoop::modifyEvent(std::shared_ptr<IOEvent> event) {
  eventCtrl->modifyEvent(event);
}

void IOEventLoop::modifyEvent(int fd) { eventCtrl->modifyEvent(fd); }
...
void IOEventLoop::runOniceAfter(const DefaultFunction callback,
                                uint32_t interval) {
  timerQueue->runOniceAfter(callback, interval);
}

void IOEventLoop::runEveryInterval(const DefaultFunction callback,
                                   uint32_t interval) {
