...
TcpConnect::TcpConnect(IOEventLoop *l, struct sockaddr_in addr, int fd)
    : clientState(NOT_LOGIN), loop(l), socketAddr(addr),
      name(socketAddr.toString()), socket(new Socket(fd)),
      event(new IOEvent(loop, fd)), state(Disconnecting) {
  setNoDelay(true);
  loop->addEvent(event);
...
void TcpConnect::setWriteCompletCallback(
    const OnWriteCompletCallback callback) {
...
TcpConnect &TcpConnect::getRefer() { return (*this); }

const SocketAddr &TcpConnect::getAddr() const { return socketAddr; }

string TcpConnect::getName() const { return name; }
...
void TcpConnect::errorEvent() { closeEvent(); }

void TcpConnect::writeEvent() {
  // loop_->assertInLoopThread();
  if (event->isWriting()) {
    /* int n = SocketOperation::write(event->getFd(), writeBuf.readIndexPtr(),
     * writeBuf.readableBytes()); */
    int n = socket->write(writeBuf.readIndexPtr(), writeBuf.readableBytes());
    if (n > 0) {
    ...
    base::Log::OUT(base::Log::Warn)
        << "Connection fd = " << std::to_string(event->getFd())
...
void TcpConnect::writeMsg(Message &msg) {
  if (msg.getLength() == 0) {
    base::Log::OUT(base::Log::Error) << "Check Message::length initlizition\n";
  }
  ...
  if (!event->isWriting() && writeBuf.isEmpty()) {
    ...
    if (n >= 0) {
      ...
      if (errno != EWOULDBLOCK) {
        base::Log::OUT(base::Log::Error) << "emmmmm : write data error";
...
void TcpConnect::setNoDelay(bool enable) { socket->setTcpNoDelay(enable); }
